...
<body onload="startTime()">
	...
	<div class="menu-bar"
		...
		<div class="title-content">Xuất danh sách CAPGRP</div>
		<form action="${pageContext.request.contextPath}/search_capgrp"
			method="post" style="position: fixed; margin-top: 75px;"
			autocomplete="off">
			<table class="form-search-iss" style="width: 100%;">
				<tr>
					<td><c:choose>
							<c:when test="${txt_keyword != ''}">
								<input autocomplete="off" name="txt_keyword"
									class="form-control" type="text" value="${txt_keyword}"
									placeholder="CAPGRP" readonly />
							</c:when>
							<c:otherwise>
								<input autocomplete="off" name="txt_keyword"
									class="form-control" type="text" placeholder="CAPGRP" />
							</c:otherwise>
						</c:choose></td>
					<td><input type="submit" class="btn btn-success"
						style="height: 30px" value="TRUY VẤN" readonly="readonly" /></td>
					<td></td>
				</tr>
			</table>
		</form>
		...
		<div class="themThongTinTraSoatISS" style="height: 50px">
			<div id="thediv_scroll"
				...
				<table rules="groups" frame="hsides" id="testTable"
					...
					<tbody>
						<c:forEach items="${list_capgrp}" var="p">
							<tr id="rows" class="rows">
								<td>${p.groupId}</td>
								<td>${p.userId}</td>
								<td>${p.username}</td>
								<td style="mso-number-format: '\@'">${p.branch}</td>
								<td><c:choose>
										<c:when test="${p.email != 'null'}">${p.email}</c:when>
									</c:choose></td>
							</tr>
						</c:forEach>
					</tbody>
